...
- Create a copy of ``workinsights_v2/settings/local.py.example``:  
  `cp workinsights_v2/settings/local.py.example workinsights_v2/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/workinsights_v2/settings/base.py``.
- Adjust ``backend/workinsights_v2/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
